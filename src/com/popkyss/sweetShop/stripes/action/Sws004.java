...
@UrlBinding("/sws004.action")
public class Sws004 extends ASessionActionSupport<Sws004Bean> {

	private static final String CURRENT_JSP = "/jsp/sweetShop/sws004.jsp";
	...
	public Resolution edit() {
		try {
			actionBean.setUzivatel(opravneni.najdiZamestnance(idZamestnanec));
	...
	public Resolution ulozitAdresu() {
		try {
			opravneni.ulozitAdresuZamestnance(actionBean.getUzivatel());
			msgI("004.02");
			editovatAdresu();
		} catch (BasicServiceException e) {
			msgE(e.getId(), e.getParams());
	...
	public Resolution ulozit() {
		try {
			opravneni.upravitZamestnance(actionBean.getUzivatel());
			msgI("004.01");
		} catch (BasicServiceException e) {
			msgE(e.getId(), e.getParams());
		}
	...
	public Sws004Bean getActionBean() {
		return actionBean;
	}

	public void setActionBean(Sws004Bean actionBean) {
